function [X,P]=ukf_corr(Xmenos,Pmenos,X1,X2,z,R,measurement_data,n_observers)
...
[z1,Z1,P2,Z2]=ut(X1,Wm,Wc,m,R,measurement_data,n_observers); %unscented transformation of measurments
...
function [y,Y,P,Y1]=ut(X,Wm,Wc,n,R,measurement_data,n_observers)   
...
ro=measurement_data.sim_ro;
quat=measurement_data.sim_qo;
rsun=measurement_data.sim_rsun;

for obv=1:n_observers

for k=1:L                   
    Y(1+(obv-1)*3:3*obv,k)=meassurement_function(X(:,k),ro(obv,:),quat(obv,:),rsun(obv,:)); 
